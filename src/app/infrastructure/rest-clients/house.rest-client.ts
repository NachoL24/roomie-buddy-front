...
import { HouseResponseDto } from '../dtos';
...
export class HouseRestClient extends HouseRepository {
    ...
    getHousesByRoomie(roomieId: number): Observable<House[]> {
        return this.http.get<HouseResponseDto[]>(`${this.apiBaseUrl}/houses/roomie/${roomieId}`)
            .pipe(map(houses => houses.map(HouseMapper.fromResponse)));
    }
