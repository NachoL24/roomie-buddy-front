...
@Component({
    ...
    template: `
    <div class="house-dashboard" *ngIf="house as h">
      <div class="header">
        <h1>{{ h.name }}</h1>
      ...
      <div class="content">
        <section class="transactions">
          <h2>Transactions</h2>
          <div class="table-wrapper" *ngIf="expenses?.length; else emptyTx">
        ...
        <aside class="members">
          ...
          <ul class="member-list">
            <li *ngFor="let m of h.members" class="member-item">
    ...
    <div *ngIf="!house" class="loading">Loading...</div>
  `,
    styles: [`
    :host { display: block; }
...
export class HouseDashboardComponent implements OnInit {
    ...
    private refresh(houseId: number) {
        this.houseService.getHouseById(houseId).subscribe(h => this.house = h);
        this.expenseService.getHouseExpenses(houseId).subscribe(ex => this.expenses = ex);
    }
